...
message("\n🔬 Running SynGO GSEA for all contrasts...")
for (co in names(contrast_tables)) {
...
mitocarta_lists <- mitocarta_gmt(here::here(config$mitocarta_file))

# Create result storage
mitocarta_gsea_results <- list()

# Run MitoCarta GSEA for each contrast (uses pre-computed contrast_tables)
message("\n🔬 Running MitoCarta GSEA for all contrasts...")
for (co in names(contrast_tables)) {
  # Close any lingering graphic devices before starting new contrast
  close_all_devices()
  ...
  mitocarta_gsea_results[[co]] <- safe_res

  # Ensure all devices are closed after each iteration
  close_all_devices()
}
message("✓ MitoCarta GSEA complete")

# ------------------------------------------------------------
# CHECKPOINT: Save MitoCarta GSEA results
# ------------------------------------------------------------
message("💾 Saving MitoCarta GSEA checkpoints...")
saveRDS(mitocarta_gsea_results, file.path(checkpoint_dir, "mitocarta_gsea_results.rds"))
saveRDS(mitocarta_lists,        file.path(checkpoint_dir, "mitocarta_lists.rds"))
message("✓ MitoCarta GSEA checkpoints saved (", length(mitocarta_gsea_results), " contrasts)")
