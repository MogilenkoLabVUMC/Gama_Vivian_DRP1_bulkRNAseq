...
|--------|---------|---------|--------|--------------|
| **9.create_master_pathway_table.py** | Create comprehensive GSEA pathway table | 2-5 min | `master_pathway_table.csv` (109K rows) | Python exports, pattern classifications |
| **9b.create_master_gsva_table.R** | Create focused GSVA table (7 key modules) | 1-2 min | `master_gsva_table.csv`, `gsva_pattern_summary.csv`, `gsva_statistics_summary.txt` | GSVA checkpoints |
| **10.comprehensive_gsva_analysis.R** | Create comprehensive GSVA table (all pathways) | 15-40 min | `master_gsva_all_pathways.csv` (87K rows) | Expression data, all databases |

**Usage**:
```bash
# Create master pathway table (GSEA results, all databases)
python3 02_Analysis/9.create_master_pathway_table.py

# Create focused GSVA table (7 key trajectory modules)
Rscript 02_Analysis/9b.create_master_gsva_table.R

# Create comprehensive GSVA table (all pathways, all databases)
Rscript 02_Analysis/10.comprehensive_gsva_analysis.R
```

**Outputs**:
- `master_pathway_table.csv`: All GSEA pathways across all contrasts with pattern classifications (109K rows)
- `master_gsva_table.csv`: GSVA scores for 7 key modules in long format with statistics (42 rows)
- `master_gsva_all_pathways.csv`: GSVA scores for all pathways across all databases (87K rows)
...
See `03_Results/02_Analysis/MASTER_TABLE_README.md` for detailed documentation.
...
9.create_master_pathway_table.py
    └── Depend on: Python exports from 3.export_gsea_for_python.R, pattern classifications

9b.create_master_gsva_table.R
    └── Depend on: GSVA checkpoint from 1a, QC variables checkpoint from 1a
```
...
| `gsva_module_scores.rds` | GSVA scores | Trajectory viz, 9b master table |
