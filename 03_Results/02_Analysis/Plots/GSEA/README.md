...
**Generated by:**
- `02_Analysis/1.1.main_pipeline.R` - Main GSEA pipeline (MSigDB + SynGO)
...
│   ├── csv_data/                 # Raw and filtered CSV data for pooled plots
│   ├── test_output/              # Test/verification plots
│   └── pooled_dotplots_summary.txt  # Generation summary
│
...
Each database folder contains up to 10 files:

### Plot Files

| File Pattern | Description |
|--------------|-------------|
| `[contrast]_[db]_dot.pdf` | Traditional GSEA dotplot (x=GeneRatio, y=Pathway) - SynGO/MitoCarta |
| `[contrast]_[db]_bar.pdf` | Bar plot visualization - SynGO/MitoCarta |
...
| `[contrast]_[db]_no_results.pdf` | Placeholder when no significant pathways found |

### Data Files

| File Pattern | Description |
|--------------|-------------|
| `GSEA_[db]_result.rds` | Raw GSEA result object (SynGO, MitoCarta) |
| `[contrast]_[db]_results.txt` | Text summary of enrichment results |

**Note:** SynGO and MitoCarta use `_dot.pdf` and `_bar.pdf` naming, while MSigDB databases use the full plot type names listed above.
...
- Files: `[contrast]_pooled_comprehensive.pdf`
- Dotplots combining top pathways from all databases
- Includes: Hallmark, KEGG, Reactome, GO:BP/CC/MF, WikiPathways, Canonical, CGP, TF, SynGO, MitoCarta

**focused/** - Hallmark + KEGG + Reactome only
- Files: `[contrast]_pooled_focused.pdf`
- More readable plots with fewer pathways
- Best for publication figures

**csv_data/** - Raw and filtered data exports
- `[contrast]_raw_[type].csv` - Unfiltered GSEA results
- `[contrast]_filtered_[type].csv` - Filtered for plotting
- `FILTERING_STRATEGY.txt` - Documentation of filtering logic

**test_output/** - Verification plots
- `TEST_pooled_dotplot.pdf` - Basic functionality test
- `TEST_gradient_verification.pdf` - Color gradient verification
- `TEST_significance_outline_verification.pdf` - Significance indicator test
- `TEST_pooled_dotplot_with_outlines.pdf` - Combined feature test

**Root files:**
- `pooled_dotplots_summary.txt` - Generation summary and statistics
...
```r
# 02_Analysis/1.3.add_mitocarta.R
...
- MitoCarta: Run `02_Analysis/1.3.add_mitocarta.R`
...
- **PDF plots:** 696
- **Text summaries:** 103
- **RDS objects:** 18 (SynGO and MitoCarta result objects)
- **CSV data exports:** 20 (Cross_database_pooled/csv_data)
- **README:** 1 (this file)

**Directory breakdown:**
- **9 contrast folders** (G32A_vs_Ctrl_D35, G32A_vs_Ctrl_D65, R403C_vs_Ctrl_D35, R403C_vs_Ctrl_D65, Time_Ctrl, Time_G32A, Time_R403C, Maturation_G32A_specific, Maturation_R403C_specific)
- **12 database subfolders per contrast** (hallmark, kegg, reactome, gobp, gocc, gomf, wiki, canon, cgp, tf, SynGO, MitoCarta)
- **1 cross-database pooled folder** with 4 subdirectories

**Typical contrast folder structure:**
```
[Contrast]/
├── hallmark/      (7 PDFs + 1 TXT)
├── kegg/          (7 PDFs + 1 TXT)
├── reactome/      (7 PDFs + 1 TXT)
├── gobp/          (7 PDFs + 1 TXT)
├── gocc/          (7 PDFs + 1 TXT)
├── gomf/          (7 PDFs + 1 TXT)
├── wiki/          (7 PDFs + 1 TXT)
├── canon/         (7 PDFs + 1 TXT or no_results.pdf)
├── cgp/           (7 PDFs + 1 TXT)
├── tf/            (7 PDFs + 1 TXT)
├── SynGO/         (9 PDFs + 1 RDS + 1 TXT)
└── MitoCarta/     (9 PDFs + 1 RDS + 1 TXT or no_results.pdf)
```
